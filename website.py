...
st.markdown("Silakan unggah file `.csv` dengan satu kolom bernama `nama_modul`, "
            "berisi daftar modul yang dipisahkan dengan koma (`,`). "
            "Contoh isi baris: `os, sys, pandas`")
...
if uploaded_file:
    ...
    if 'nama_modul' not in df.columns:
        st.error("Kolom 'nama_modul' tidak ditemukan dalam file CSV.")
        st.stop()
    ...
    try:
        ...
        st.error("Terjadi error memori. Coba naikkan nilai support atau kurangi data.")
        st.stop()

    if freq_items.empty:
        st.warning("Tidak ada itemset yang memenuhi nilai support.")
        st.stop()

    rules = association_rules(freq_items, metric="confidence", min_threshold=confidence)

    # Hanya ambil aturan 1 ke 1
    rules = rules[
    ...
    node_color = [
        rules[rules['Modul A'] == node]['Confidence'].sum()
        if node in rules['Modul A'].values else 0
        for node in G.nodes()
    ]
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(
            title="Visualisasi Jaringan Asosiasi Modul",
            title_x=0.4,
            showlegend=False,
            hovermode='closest',
